...
export default function DocumentUpload({ onDocumentUploaded, lessonId }: DocumentUploadProps) {
  ...
  const uploadFile = async (file: File) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      setError("PDF dosyası 50MB'dan küçük olmalıdır")
      return
    }
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "PDF yüklenirken hata oluştu"
      ...
      >
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="space-y-1">
            <p className="text-gray-300 font-medium">
              {success ? "Yükleme Tamamlandı" : "PDF Dosyasını Sürükle ve Bırak"}
            </p>
            {!success && (
              <p className="text-gray-500 text-sm">
                veya gözatmak için tıkla
              </p>
            )}
          </div>
        ...
        {dragActive && (
          <div className="absolute inset-0 bg-orange-500/10 rounded-xl flex items-center justify-center backdrop-blur-sm">
            <p className="text-orange-500 font-bold text-lg">Dosyayı Buraya Bırak</p>
          </div>
        )}
      </div>
    </div>
