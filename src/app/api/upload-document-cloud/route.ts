...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > 50 * 1024 * 1024) {
      return NextResponse.json({ error: "PDF dosyası 50MB'dan küçük olmalıdır" }, { status: 400 })
    }
    ...
    return NextResponse.json({
      ...
      message: "PDF başarıyla cloud'a yüklendi"
    })

  } catch (error) {
    console.error("Cloud document upload error:", error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Belge yüklenirken hata oluştu" },
      { status: 500 }
    )
  }
}
