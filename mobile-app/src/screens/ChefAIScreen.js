...
export default function ChefAIScreen() {
    ...
    useFocusEffect(
        ...
        }, [messages]) // Depend on messages to capture current state
    );
    ...
    const loadHistoryItem = (item) => {
        // If there's a current unsaved chat, save it first? 
        // Logic: tapping a history item replaces current view.
        if (messages.length > 0) {
            // Optional: Save current before loading old one?
            // For simplicity, we just overwrite as per "switching tabs resets" logic, 
            // here we assume user wants to view this old chat.
        }
